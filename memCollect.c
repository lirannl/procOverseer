...
memEntry_t *
create_newEntry(memEntry_t *entry, pid_t newPID, char *newTime, int newBytes, char *newFile, char *newArgs) {
    entry->pid = newPID;
    entry->time = newTime;
    ...
    for (int i = 0; i < 5; i++) {
        for (; head != NULL; head = head->next) {
            if (head->entry->pid == pid[i]) {
                sprintf(memArray, "\n%d %d %s %s", head->entry->pid, head->entry->bytes, head->entry->file,
                        head->entry->args);
